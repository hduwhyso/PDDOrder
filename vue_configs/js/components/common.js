...
let mixin_common = {
  ...
  methods: {
    ...
    doSaveConfigs: function (deleteFields) {
      ...
      // 如果存在 orderMap，确保不覆盖它
      if (this.configs.orderMap) {
        newConfigs.orderMap = this.configs.orderMap;
      }
      ...
      if (deleteFields && deleteFields.length > 0) {
        deleteFields.forEach(key => {
          ...
          // 如果 newConfigs 里有 orderMap，就删除它
      if (newConfigs.hasOwnProperty('orderMap')) {
          delete newConfigs.orderMap;
      }
      if (newConfigs.hasOwnProperty('currentOrder')) {
          delete newConfigs.currentOrder;
      }
  ...
  mounted () {
    console.log('加载当前界面配置')
    this.loadConfigs()
    this.persistCurrentFields()
  },
  destroyed () {
    // this.$toast.success('保存当前界面配置');
