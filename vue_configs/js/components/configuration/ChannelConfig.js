const ChannelConfig = {
	...
	computed: {
	  ...
	  currentPayWayOptions(){
		if(this.configs.dict?.paymentPlatform?.length){
			let array=this.configs.dict?.paymentPlatform.filter(payment => this.configs.platform[this.currentPlatformKey].payWayOptions.includes(payment.name));
			if(array.length==1){
	...
	methods: {
		get_payList() {
		  return API.post('http://kk.smartapp.cc:8001/open/feedov/bind/getTargets',{"name":this.configs.platform[this.currentPlatformKey].owner,"platforms": this.configs.platform[this.currentPlatformKey].payWay,"accountID":this.configs.platform[this.currentPlatformKey].accountID}).then(resp => {
			if(resp.data?.length){
