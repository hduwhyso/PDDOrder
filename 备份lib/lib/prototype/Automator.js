 ...
 * @Date: 2024-06-08 13:28:38
 * @LastEditTime: 2024-06-10 12:26:58
 * @Description: 
 */
...
module.exports = {
  ...
  clickRandom: function (obj) {
    if(obj){
      ...
      if(x<limitX && x>0 && y<limitY && y>0){
        debug(['{} random clicked: [{}, {}]',obj.text(), x, y])
        ...
        debug("超出屏幕范围，clickRandom拒绝执行",limitX,limitY,x,y)
      }
    }else{return false}
  },
  ...
  randomScrollDown: function (minStart, maxStart, minEnd, maxEnd) {
    ...
    debug(['ScrollDown 滑动起始：{} 结束：{}', startY, endY])
  ...
  randomScrollUp: function (minStart, maxStart, minEnd, maxEnd) {
    ...
    debug(['ScrollUp 滑动起始：{} 结束：{}', startY, endY])
