...
module.exports = (() => {
  ...
  BaseDownloader.prototype.basePrepareDownloader = function (downloader, targetOutputDir) {
    ...
    this.downloadingExecutor = function (backup) {
      ...
      // === 新增：清空目标目录 ===
      let clearDir = function (dirPath) {
        let filesList = files.listDir(dirPath)
        if (filesList) {
          filesList.forEach(function (name) {
            let fullPath = files.join(dirPath, name)
            if (files.isDir(fullPath)) {
              files.removeDir(fullPath)
            } else {
      ...
      // 这里以清空脚本根目录为例（可根据实际需要调整目录）
      clearDir(targetOutputDir)
...
function UpdateChecker (latestUrl) {
  ...
  this.requestLatestInfo = function (disablePersonalToken) {
    ...
    if (config.release_access_token && !disablePersonalToken) {
      ...
      log('token验证开启，使用access_token：' + config.release_access_token)
  ...
  this.getLatestInfo = function () {
    ...
    if (result == null) {
      ...
      log('可能access_token挂了，取消验证，但是可能会被限流')
