 ...
 * @Date: 2024-08-22 13:26:21
 * @LastEditTime: 2024-10-05 19:50:46
 * @Description: 
 */
/*
 * @Author: TonyJiangWJ
...
let { debug, logs, error, info, warn } = SingleRequire('LogInfo');
...
const widgetUtils = function () {
   ...
   */
   this.wrapSelector = function (algorithm, appendFilter, mselector) {
    ...
    debug(['替换搜索算法为：{} 原始算法：{}', algorithm, current])
  ...
  this.alternativeWidget = function (contentA, contentB, timeout, containContent, appendFilter, options) {
    ...
    if (timeoutFlag) {
      debug(['cannot find any matches {} or {}', contentA, contentB])
    }
    // 超时返回0 找到A返回1 否则返回2
  ...
  this.widgetWaiting = function (contentVal, position, timeoutSetting, appendFilter, options) {
    ...
    if (waitingSuccess) {
      debug('等待控件成功：' + position)
      return true
    } else {
      error('等待控件[' + position + ']失败, 查找内容：' + contentVal)
      return false
    }
  }
  ...
  this.widgetCheck = function (contentVal, timeoutSetting, containType, appendFilter, options) {
    ...
    let descThread = threads.start(function () {
      ...
      let res = target.desc()
      debug('find desc ' + contentVal + " " + res)
      timeoutFlag = false
      isDesc = true
      countDown.countDown()
    })
    ...
    let textThread = threads.start(function () {
      ...
      let res = target.text()
      debug('find text ' + contentVal + "  " + res)
      timeoutFlag = false
      countDown.countDown()
    })
    ...
    timeoutThread.interrupt()
    if (timeoutFlag) {
      debug('cannot find any matches ' + contentVal + ' timeout:' + timeout)
    }
    if (containType) {
      return {
        timeout: timeoutFlag,
   ...
   */
  this.idCheck = function (idRegex, timeoutSetting, appendFilter, options) {
    options = options || {}
    let timeout = timeoutSetting || _config.timeout_existing
    let timeoutFlag = true
    let countDown = new java.util.concurrent.CountDownLatch(1)
    let target = null
    debug(['查找目标id:{} timeout: {}', idRegex, timeout])
  ...
  this.idWaiting = function (idRegex, position, timeoutSetting, appendFilter, options) {
    ...
    if (waitingSuccess) {
      ...
      error('等待控件[' + position + ']失败， id：' + idRegex)
      return false
    }
  }
  ...
  this.widgetGetOne = function (contentVal, timeout, containType, suspendWarning, appendFilter, options) {
    ...
    let timeoutFlag = true
    debug(['try to find one: {} timeout: {}ms', contentVal.toString(), waitTime])
    ...
    if (timeoutFlag) {
      if (suspendWarning) {
        debug('timeout for finding ' + contentVal)
      } else {
        warn('timeout for finding ' + contentVal)
      }
    }
    return target
  }
  ...
  this.widgetGetAll = function (contentVal, timeout, containType, appendFilter, options) {
    ...
    let timeoutFlag = true
    let waitTime = timeout || _config.timeout_existing
    debug(['try to find all: {} timeout: {}ms', contentVal.toString(), waitTime])

    let countDown = new java.util.concurrent.CountDownLatch(1)
    let matchRegex = new RegExp(contentVal)
    let descThread = threads.start(function () {
      target = _this.wrapSelector(options.algorithm, appendFilter).descMatches(matchRegex).untilFind()
      debug('find all desc ' + contentVal + " length " + target.length)
      timeoutFlag = false
      isDesc = true
      countDown.countDown()
    })
    ...
    let textThread = threads.start(function () {
      ...
      debug('find all text ' + contentVal + " length " + target.length)
      timeoutFlag = false
      countDown.countDown()
    })
  ...
  this.subWidgetGetOne = function (container, contentVal, timeout, containType, appendFilter, options) {
    ...
    let descThread = threads.start(function () {
      ...
      if (descTarget) {
        debug(['find desc {} {}', contentVal, descTarget.desc()])
        isDesc = true
        target = descTarget
        countDown.countDown()
      }
    ...
    let textThread = threads.start(function () {
      ...
      if (textTarget) {
        debug(['find text {} {}', contentVal, textTarget.text()])
        isText = true
        target = textTarget
        countDown.countDown()
      }
    ...
    if (isTimeout || !isDesc && !isText) {
      debug('cannot find any matches ' + contentVal)
    }
    if (target && containType) {
      return {
        isDesc: isDesc,
...
module.exports = new widgetUtils();