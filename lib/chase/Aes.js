 ...
 * @Description: AES加密工具,增加AES-CBC-Pkcs7加解密（encrypt_CBC，decrypt_CBC）
 */
...
AesUtil.prototype.encrypt_CBC = function (message, key, iv) {
  key = key || this.key
  if (key.length !== 8 && key.length !== 16 && key.length !== 32) {
    console.error('密码长度不正确必须为8/16/32位')
    return null
...
AesUtil.prototype.decrypt_CBC = function(encrypt, key, iv) {
  key = key || this.key
  if (key.length !== 8 && key.length !== 16 && key.length !== 32) {
    console.error('密码长度不正确必须为8/16/32位')
    return null
